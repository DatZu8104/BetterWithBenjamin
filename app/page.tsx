...
interface Word { id: string; english: string; definition: string; type: string[]; group: string; }
...
export default function Home() {
  ...
  // ... (Giữ nguyên loadData, useEffect, filter logic, sort logic...)
  const loadData = async () => {
    ...
    let settingsArr: any[] = [];
    if (db.groupSettings) settingsArr = await db.groupSettings.toArray();
    ...
    if (db.folders) {
        ...
        sortedFolders.forEach(f => { if (f.color) colorMap[f.name] = f.color; });
  ...
  const handleCreateFolder = async (folderName: string, color: string = 'blue') => {
    ...
    try {
      await db.folders.put({ name: folderName, color: color, createdAt: Date.now() });
      await loadData(); setCurrentFolder(folderName);
  ...
  const handleUpdateFolder = async (oldName: string, newName: string, newColor: string) => {
    ...
    try {
        if (oldName !== newName) {
            ...
            const affectedGroups = await db.groupSettings.where('folder').equals(oldName).toArray();
            for (const g of affectedGroups) {
                await db.groupSettings.update(g.groupName, { folder: newName });
            }
        }
        const existing = await db.folders.get(oldName === newName ? oldName : newName);
  ...
  const handleDeleteFolder = async (folderName: string) => {
    if (!db.folders) return;
    try {
        // 1. Xóa folder trong DB
        await db.folders.delete(folderName);
        
        // 2. Tìm các group đang ở trong folder này và đưa ra ngoài (root)
        const affectedGroups = await db.groupSettings.where('folder').equals(folderName).toArray();
        for (const g of affectedGroups) {
            await db.groupSettings.update(g.groupName, { folder: "" });
        }

        // 3. Reload và thoát ra ngoài
        await loadData();
  ...
  // ... (Giữ nguyên các hàm add/edit/delete word/group, export/import...)
  ...
  const handleAddGroup = () => {
    ...
    if (currentFolder) handleMoveGroup(name, currentFolder);
    else { db.groupSettings.put({ groupName: name, folder: "" }); loadData(); }
  };
  ...
  const handleExport = async () => {
    ...
    const blob = new Blob([JSON.stringify({ exportedAt: new Date(), groups: [...new Set(words.map((w) => w.group))], words, learned: learned.map((x) => x.id), groupSettings: settings, folders }, null, 2)], { type: 'application/json' });
  ...
  const handleResetLearnContext = async () => {
    ...
    if (learnMode) { setIsResetting(true); setCurrentWord(null); setTimeout(() => { pickRandomWord(contextWords, newLearnedIds); setIsResetting(false); }, 80); }
  };
      ...
      <main className="flex-1 overflow-hidden relative">
        {learnMode ? (
          ...
          <GroupListView 
            ...
            onDeleteFolder={handleDeleteFolder} // <--- TRUYỀN HÀM XÓA
