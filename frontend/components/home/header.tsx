...
import { api } from '../../lib/api'; // Đảm bảo import đúng
...
interface HeaderProps {
  ...
  totalWords?: number;
  learnedCount?: number;
  // Các props onExport, onImport cũ có thể bỏ qua vì ta xử lý trực tiếp ở đây
  onExport?: () => void;
  onImport?: (e: any) => void;
}
...
export function Header({ 
  ...
  const [isProcessing, setIsProcessing] = useState(false); // Loading khi import/export
  
  const percentage = totalWords > 0 ? Math.round((learnedCount / totalWords) * 100) : 0;

  // --- XỬ LÝ EXPORT (XUẤT FILE) ---
  const handleExportClick = async () => {
    try {
        ...
        link.href = url;
        link.download = `backup_flashcards_${new Date().toISOString().slice(0,10)}.json`;
  ...
  // --- XỬ LÝ IMPORT (NHẬP FILE) ---
  const triggerImport = () => {
      // Kích hoạt nút input file ẩn
      ...
      try {
          ...
          reader.onload = async (e) => {
              try {
                  ...
                  window.location.reload(); // Tải lại trang để cập nhật data mới
              } catch (err) {
            ...
            <div className="absolute right-0 top-14 w-72 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl z-50 p-1 text-zinc-300 animate-in fade-in zoom-in-95 duration-200 ring-1 ring-black/5">
                ...
                <div className="p-1 space-y-0.5">
                    ...
                    <button 
                        ...
                        className="w-full text-left px-3 py-2.5 text-sm hover:bg-zinc-800 rounded-lg flex gap-3 items-center transition-colors text-zinc-300 disabled:opacity-50"
                    >
                    ...
                    >
                        ...
                        {isProcessing ? "Đang tải xuống..." : "Export dữ liệu"}
                    </button>
                </div>
