...
export function BulkForm({ group, onDone }: { group: string; onDone: () => void; }) {
  ...
  const handlePreview = () => {
    ...
    if (badLines.length > 0) setError(`Line error: "${badLines[0].slice(0, 40)}..."`);
    else setError(null);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      console.error(err); setError('Error while saving.');
    } finally {
  ...
  return (
    <div className="mt-4 border rounded p-4 bg-muted/5">
      <p className="mb-2 text-sm text-muted-foreground">Paste the list (word - def - type). Automatically switch normally.</p>
      <textarea
      ...
      {preview && (
        <div className="mt-4">
          <p className="text-sm mb-2">Find {preview.length} từ:</p>
